...
{
	--曌雲裳
	--对三行目标造成 186% 伤害      剑阁之主    技能释放时自身攻击+10%，防御+10%，回避+10%，命中+10%，暴击+10%，格挡+10%（可叠加）
	-- 名称
		name = '曌雲裳',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100074.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		--local buff = g_skill_100074.buffs[1];
		local attack_parameter = g_skill_100074.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100074.grow;
			
		local buff_trigger_rate = g_skill_100074.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(g_skill_100074.buffs[1].buff_id, unit.id, 100074, g_skill_100074.buffs[1]);
				unit:add_buff(g_skill_100074.buffs[2].buff_id, unit.id, 100074, g_skill_100074.buffs[2]);
				unit:add_buff(g_skill_100074.buffs[3].buff_id, unit.id, 100074, g_skill_100074.buffs[3]);
				unit:add_buff(g_skill_100074.buffs[4].buff_id, unit.id, 100074, g_skill_100074.buffs[4]);
				unit:add_buff(g_skill_100074.buffs[5].buff_id, unit.id, 100074, g_skill_100074.buffs[5]);
				unit:add_buff(g_skill_100074.buffs[6].buff_id, unit.id, 100074, g_skill_100074.buffs[6]);
			end   
		end
