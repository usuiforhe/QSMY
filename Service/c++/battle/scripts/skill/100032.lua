...
{
	--嗜杀者
	--对目标单体造成 316%伤害    七魄	随机给予目标一个DEBUFF，持续2回合
	-- 名称
		name = '嗜杀者',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100032.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100032.buffs[1];
		...
		local attack_parameter = g_skill_100032.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100032.grow;
		local buff_trigger_rate = g_skill_100032.buff_trigger_rate;
	...
	--技能特殊效果	随机添加DEBUFF
		...
		if( unit.up < 4 ) then
			...
			--恶鬼罗刹效果
				...
				target:add_buff(g_skill_100032.buffs[rannum].buff_id, unit.id, 100032, g_skill_100032.buffs[rannum]);
				end
			end   
		end
