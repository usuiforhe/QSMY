...
{
	--宵
	--对目标单体造成 328%伤害     血宵蚀月	攻击后额外造成目标 10% 当前生命值的伤害
	--特效屏蔽
	-- 名称
		name = '宵',
	-- 优先级
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100012.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100012.buffs[1];
		...
		local attack_parameter = g_skill_100012.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100012.grow;
		local buff_trigger_rate = g_skill_100012.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if target.sid > 10001000 and target.sid < 10010000 then
				...
				local damage= (unit.attack- target.defense) *( attack_parameter  +  grow * (skill_lv-1))*(1-target.skill_resist/100)  + target.hp * 0.1   + unit.event_hurt - target.event_injury      ;	
				target:taking_damage(unit, damage, false); 
			end
