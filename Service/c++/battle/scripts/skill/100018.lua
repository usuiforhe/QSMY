...
{
	-- 轩辕不败
	--对前排全体造成 186% 伤害，50%概率触发无敌，持续1回合	玄绝流	技能100%触发无敌，持续1回合
	-- 名称
		name = '轩辕不败',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100018.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100018.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100018.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100018.grow;
		local buff_trigger_rate = g_skill_100018.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(buff.buff_id, unit.id, 100018, buff);
			end
		end
