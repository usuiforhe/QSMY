...
{
	-- 赭杉军
	--对前排全体造成 180% 伤害  	破玄指	无视格挡 且 必中，持续2回合
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100020.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100020.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100020.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100020.grow;
		local buff_trigger_rate = g_skill_100020.buff_trigger_rate;
		...
		if ( flag == false ) then
			for id,v in pairs(targets2) do
				local damage= (unit.attack- v.defense) *( attack_parameter  +  grow * (skill_lv-1))*(1-target.skill_resist/100)   + unit.event_hurt - target.event_injury  ;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(g_skill_100020.buffs[1].buff_id, unit.id, 100020, g_skill_100020.buffs[1]);
				unit:add_buff(g_skill_100020.buffs[2].buff_id, unit.id, 100020, g_skill_100020.buffs[2]);
			end   
		end
