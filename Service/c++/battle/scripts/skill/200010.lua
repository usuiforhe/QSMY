...
{
	--问天谴&&四非凡人
	--对全体造成伤害，并且对剩余血量低于30%的英雄附加恐惧（攻击力下降）状态，持续1回合    地狱岛主	
	-- 名称
		name = '地狱岛主',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_200010.trigger_rate;
		...
		local s1 = unit.force:get_unit_by_sid_all(11);
		local s2 = unit.force:get_unit_by_sid_all(41);
	...
	effect = function(frames, level, unit , attacker)
		...
		local buff = g_skill_200010.buffs[1];
		local buff_trigger_rate = g_skill_200010.buff_trigger_rate;
		...
		local members = {unit.force:get_unit_by_sid_all(11),unit.force:get_unit_by_sid_all(41)};
		...
		local attack_parameter = g_skill_200010.attack_parameter;
		local grow =  g_skill_200010.grow;
		...
		if buff_trigger == true then
			while i <= total do
				...
				if target.hp < target.max_hp * 0.3 then
					target:add_buff(buff.buff_id, unit.id, 200010, buff);
				end
				i = i + 1;
			end 
		end
