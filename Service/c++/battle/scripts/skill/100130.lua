...
{
	--跩鸭
	--对所有目标造成98% 伤害    神魔之兽   技能释放时友方所有增加5%攻击，持续2回合
	--特效屏蔽
	-- 名称
		name = '跩鸭',
	-- 优先级
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100130.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100130.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100130.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100130.grow;
		local buff_trigger_rate = g_skill_100130.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				...
				while i <= total2 do
					...
					targetforce:add_buff(buff.buff_id, unit.id, 100130, buff);	
					i = i + 1;
				end 
			end 	
		end
