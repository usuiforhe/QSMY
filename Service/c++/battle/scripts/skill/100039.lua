...
{
	-- 曼无歆		
	--对三行目标造成 168% 伤害		寂尘归心	将伤害30%转化为生命值
	-- 名称
		name = '曼无歆',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100039.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100039.buffs[1];		
		local attack_parameter = g_skill_100039.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100039.grow;
				
		local buff_trigger_rate = g_skill_100039.buff_trigger_rate;
	  ...
	  elseif unit.position <= 6 then 
		...
		if( unit.up < 4) then
			...
			unit:heal(unit,count/i*0.3 , true);			
		end
