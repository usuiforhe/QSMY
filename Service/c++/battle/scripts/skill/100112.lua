g_skill_100112 =
{
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100112.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100112.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100112.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100112.grow;
		local buff_trigger_rate = g_skill_100112.buff_trigger_rate;
