...
{
	--孽角
	--对随机3个目标造成 175% 伤害     孽之暴	技能释放时自身增加30%防御，持续2回合
	-- 名称
		name = '孽角',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100086.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100086.buffs[1];
		...
		local attack_parameter = g_skill_100086.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100086.grow;
		local buff_trigger_rate = g_skill_100086.buff_trigger_rate;
	...
	--攻击范围
			...
			if( unit.up < 4 ) then
				...
				if buff_trigger == true then
					unit:add_buff(buff.buff_id, unit.id, 100086, buff);
				end
			end					
    			i = i + 1;
    		end
