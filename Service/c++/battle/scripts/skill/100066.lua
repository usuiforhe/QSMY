...
{
	--白雪飄
	--对所有目标造成 92% 伤害   万法归元	技能释放时将伤害30%转化为生命值
	-- 名称
		name = '白雪飄',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100066.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100066.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100066.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100066.grow;
		local buff_trigger_rate = g_skill_100066.buff_trigger_rate;
...
--[[		buff_trigger = script_check_prob(buff_trigger_rate);
		...
		if( unit.up < 4 ) then
			...
			local healdose= (unit.attack- target.defense) *( attack_parameter  +  grow * (skill_lv-1))*(1-target.skill_resist/100)  + unit.event_hurt - target.event_injury  ;		
			unit:heal(unit, healdose * 0.3 , true);			
		end	
