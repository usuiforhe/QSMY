...
{
	--秦假仙
	--对全体造成 67% 伤害		半壁山河	友方所有增加10%防御，持续2回合
	-- 名称
		name = '秦假仙',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100058.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100058.buffs[1];
		local target = unit;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100058.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100058.grow;
		local buff_trigger_rate = g_skill_100058.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				...
				while kk <= total2 do
					...
					targetforce:add_buff(buff.buff_id, unit.id, 100058, buff);	
					kk = kk + 1;
				end --while
			end --if
		end
