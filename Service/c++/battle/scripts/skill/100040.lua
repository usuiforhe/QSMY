...
{
	--良峰贞义
	--对目标单体造成 304%伤害    刀随人意	格挡 +30%，持续2回合
	-- 名称
		name = '良峰贞义',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100040.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100040.buffs[1];
		...
		local attack_parameter = g_skill_100040.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100040.grow;
		local buff_trigger_rate = g_skill_100040.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(buff.buff_id, unit.id, 100040, buff);
			end
		end
