...
{
	-- 魚紋老鋏
	--对前排全体造成 168% 伤害 秋水横波  技能释放时使目标防御 -20%，持续2回合
	-- 名称
		name = '魚紋老鋏',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100078.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100078.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100078.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100078.grow;
		local buff_trigger_rate = g_skill_100078.buff_trigger_rate;
			...
			for id,v in pairs(targets2) do
				...
				buff_trigger = script_check_prob(buff_trigger_rate);
					if buff_trigger == true then
						v:add_buff(buff.buff_id, unit.id, 100078, buff);
					end
				end			
