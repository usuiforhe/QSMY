...
{
	--樓無痕
	--对所有目标造成98% 伤害    行云走影	技能释放后自身回避率+25% ，持续2回合
	--特效屏蔽
	-- 名称
		name = '行云走影',
	-- 优先级
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100075.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100075.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100075.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100075.grow;
		local buff_trigger_rate = g_skill_100075.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(buff.buff_id, unit.id, 100075, buff);
			end		
		end
