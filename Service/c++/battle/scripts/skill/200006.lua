...
{
	--九祸&&银煌朱武
	--对所有目标造成143% 伤害    魔界之主	所有目标防御下降10%，持续2回合
	-- 名称
		name = '魔界之主',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_200006.trigger_rate;
		...
		local s1 = unit.force:get_unit_by_sid_all(6);
		local s2 = unit.force:get_unit_by_sid_all(7);
	...
	effect = function(frames, level, unit , attacker)
		local buff = g_skill_200006.buffs[1];
		local target = unit.target;	
		if target == nil then
		    return;
		end

		local buff_trigger_rate = g_skill_200006.buff_trigger_rate;
		local buff_trigger = false ;

		local members = {unit.force:get_unit_by_sid_all(6),unit.force:get_unit_by_sid_all(7)};
		...
		local attack_parameter = g_skill_200006.attack_parameter;
		local grow =  g_skill_200006.grow;
		...
		local units = unit.force.opponent:get_unit_list_table(true);
			...
			target:add_buff(buff.buff_id, unit.id, 200006, buff);
    			i = i + 1;
    		end
