...
{
	-- 安羅伽（暗）		
	--对三行目标造成 186% 伤害	黑羽之剑	   技能释放时如生命<50%恢复自身最大生命值的50%，如生命>50%则伤害+20%
	-- 名称
		name = '安羅伽（暗）',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100129.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)
		local buff = g_skill_100129.buffs[1];		
		local attack_parameter = g_skill_100129.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100129.grow;
			
		local buff_trigger_rate = g_skill_100129.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if( unit.hp > (unit.max_hp*0.5) ) then
				...
				for id,target in pairs(targets) do
					local damage= (unit.attack- target.defense) *( attack_parameter  +  grow * (skill_lv-1)+0.2)*(1-target.skill_resist/100)  + unit.event_hurt - target.event_injury   ;	
					target:taking_damage(unit,damage ,false);
				end
			else
				...
				local healself=unit.max_hp * 0.5 ; 
				unit:heal(unit,healself, true);
			end
