...
{
	--一页书
	--对目标单体造成 352%伤害      破甲尖峰七旋指	随机给予目标2个DEBUFF，持续2回合
	-- 名称
		name = '一页书',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100002.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit, attacker)	
		local buff = g_skill_100002.buffs[1];
		...
		local attack_parameter = g_skill_100002.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100002.grow;
		local buff_trigger_rate = g_skill_100002.buff_trigger_rate;
	...
	--技能特殊效果	随机添加DEBUFF
		...
		if( unit.up < 4 ) then
			...
			local rannum1 = math.random(7);
			local rannum2 = math.random(7);
			...
			if target.sid > 10001000 and target.sid < 10010000 then
				...
				target:add_buff(g_skill_100002.buffs[rannum1].buff_id, unit.id, 100002, g_skill_100002.buffs[rannum1]);
				target:add_buff(g_skill_100002.buffs[rannum2].buff_id, unit.id, 100002, g_skill_100002.buffs[rannum2]);
			end
			end   
		end
