...
{
	-- 冲田鹰司
	--对前排全体造成 168% 伤害  	飞雪势	30%概率触发冰冻效果，持续2回合
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100030.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100030.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100030.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100030.grow;
		local buff_trigger_rate = g_skill_100030.buff_trigger_rate;
			...
			for id,v in pairs(targets2) do
				...
				if( unit.up < 4 ) then
					...
					if target.sid > 10001000 and target.sid < 10010000 then
						...
						v:add_buff(buff.buff_id, unit.id, 100030, buff);
					end
					end   
				end
			end
