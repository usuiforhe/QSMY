...
{
	-- 六祸苍龙
	--对前排全体造成 180% 伤害  	紫龙战麟	， +50%格挡率 +15%免伤，持续2回合
	-- 名称
		name = '六祸苍龙',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100019.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100019.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100019.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100019.grow;
		local buff_trigger_rate = g_skill_100019.buff_trigger_rate;
		...
		--[[		local unitsforce = unit.force:get_unit_list_table(true);
				...
				while kk <= total2 do
					...
					unit:add_buff(g_skill_100019.buffs[1].buff_id, unit.id, 100019, g_skill_100019.buffs[1]);	
					unit:add_buff(g_skill_100019.buffs[2].buff_id, unit.id, 100019, g_skill_100019.buffs[2]);	
			--		kk = kk + 1;
			--	end 
			end --if
		end
