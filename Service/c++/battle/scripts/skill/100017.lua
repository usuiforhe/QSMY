...
{
	-- 源武藏		
	--对三行目标造成 186% 伤害		 返无·归一	被攻击时40%概率使对方眩晕1回合
	-- 名称
		name = '源武藏',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100017.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100017.buffs[1];		
		local attack_parameter = g_skill_100017.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100017.grow;
				
		local buff_trigger_rate = g_skill_100017.buff_trigger_rate;
...
--[[
		if( unit.up < 4 ) then
			...
			buff_trigger = script_check_prob(buff_trigger_rate);
			if buff_trigger == true then
				unit:add_buff(buff.buff_id, unit.id, 100017, buff);
