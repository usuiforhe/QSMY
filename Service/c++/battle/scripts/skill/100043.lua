...
{
	--屈世途
	--对目标单体造成 316%伤害    碎心掌	目标防御下降20%，持续2回合
	-- 名称
		name = '屈世途',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100043.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		...
		local buff = g_skill_100043.buffs[1];
		...
		local attack_parameter = g_skill_100043.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100043.grow;
		local buff_trigger_rate = g_skill_100043.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				target:add_buff(buff.buff_id, unit.id, 100043, buff);
			end
		end
