...
{
	--伊达我流&&天草二十六
	--对敌方单体造成400%伤害    双剑连击	自己暴击+15%，持续2回合
	-- 名称
		name = '双剑连击',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_200002.trigger_rate;
		...
		local s1 = unit.force:get_unit_by_sid_all(33);
		local s2 = unit.force:get_unit_by_sid_all(38);
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_200002.buffs[1];
		...
		local members = {unit.force:get_unit_by_sid_all(33),unit.force:get_unit_by_sid_all(38)};
		...
		local attack_parameter = g_skill_200002.attack_parameter;
		local grow =  g_skill_200002.grow;
		local buff_trigger_rate = g_skill_200002.buff_trigger_rate;
		...
		if buff_trigger == true then
			unit:add_buff(buff.buff_id, unit.id, 200002, buff);
		end   
