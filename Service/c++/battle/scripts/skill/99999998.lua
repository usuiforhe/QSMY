...
{
	-- 问天谴
	--对前排全体造成 174% 伤害  	荡乾坤	目标防御下降20%，持续2回合
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_99999998.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_99999998.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_99999998.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_99999998.grow;
		local buff_trigger_rate = g_skill_99999998.buff_trigger_rate;
			...
			for id,v in pairs(targets2) do
				...
				if( unit.up < 4 ) then
					...
					if buff_trigger == true then
						v:add_buff(buff.buff_id, unit.id, 99999998, buff);
					end
				end
			end	
		else
