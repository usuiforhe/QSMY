...
{
	--兒道茲
	--对随机3个目标造成 163% 伤害     羅布之子	攻击+10%，格挡+10%
	-- 名称
		name = '兒道茲',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100116.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100116.buffs[1];
		...
		local attack_parameter = g_skill_100116.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100116.grow;
		local buff_trigger_rate = g_skill_100116.buff_trigger_rate;
	...
	--攻击范围
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(g_skill_100116.buffs[1].buff_id, unit.id, 100113, g_skill_100116.buffs[1]);
				unit:add_buff(g_skill_100116.buffs[2].buff_id, unit.id, 100113, g_skill_100116.buffs[2]);
			end    
		end
