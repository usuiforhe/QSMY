...
{
	--泰逢
	--对随机3个目标造成 169% 伤害     回元心诀	技能释放后自身回避率+25% ，持续2回合
	-- 名称
		name = '泰逢',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100061.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100061.buffs[1];
		...
		local attack_parameter = g_skill_100061.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100061.grow;
		local buff_trigger_rate = g_skill_100061.buff_trigger_rate;
	...
	--攻击范围
			...
			if( unit.up < 4 ) then
				...
				if buff_trigger == true then
					unit:add_buff(buff.buff_id, unit.id, 100061, buff);
				end
			end					
    			i = i + 1;
    		end
