...
{
	--尹秋君
	--对目标单体造成 328%伤害      紫云之涛	     技能释放时将伤害30%转化为生命值
	-- 名称
		name = '尹秋君',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100070.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit, attacker)	
		local buff = g_skill_100070.buffs[1];
		...
		local attack_parameter = g_skill_100070.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100070.grow;
		local buff_trigger_rate = g_skill_100070.buff_trigger_rate;
		...
		if( unit.up < 4) then
			...
			unit:heal(unit,damage*0.3, true);			
		end
