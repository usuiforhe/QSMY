...
{
	--霏嬰
	--对随机3个目标造成 163% 伤害       梦蝶无痕	如当前生命比目标单位高，那么伤害+20%
	-- 名称
		name = '霏嬰',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100077.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100077.buffs[1];
		...
		local attack_parameter = g_skill_100077.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100077.grow;
		local buff_trigger_rate = g_skill_100077.buff_trigger_rate;
