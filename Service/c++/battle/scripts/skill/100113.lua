g_skill_100113 =
{
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100113.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100113.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100113.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100113.grow;
		local buff_trigger_rate = g_skill_100113.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(g_skill_100113.buffs[1].buff_id, unit.id, 100113, g_skill_100113.buffs[1]);
				unit:add_buff(g_skill_100113.buffs[2].buff_id, unit.id, 100113, g_skill_100113.buffs[2]);
			end		
		end
