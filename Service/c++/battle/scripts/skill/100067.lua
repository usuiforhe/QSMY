...
{
	--黃商子
	--对全体造成 61% 治疗		道凌太一	60%概率对所有友方防御+5%，持续2回合
	-- 名称
		name = '黃商子',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100067.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100067.buffs[1];
		local target = unit;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100067.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100067.grow;
		local buff_trigger_rate = g_skill_100067.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				...
				while kk <= total2 do
					...
					targetforce:add_buff(buff.buff_id, unit.id, 100067, buff);	
					kk = kk + 1;
				end --while
			end --if
		end
