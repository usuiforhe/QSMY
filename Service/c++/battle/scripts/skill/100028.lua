...
{
	--淵姬
	--对全体造成 67% 治疗		神遗秘术	 	60%概率对随机3个单位释放恢复术，持续2回合
	-- 名称
		name = '淵姬',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100028.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100028.buffs[1];
		local target = unit;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100028.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100028.grow;
		local buff_trigger_rate = g_skill_100028.buff_trigger_rate;
		...
		if ( total <= 3 ) then
			...
			elseif (buff_trigger == true) then
				while ( healnum <= total ) do	
					...
					target:add_buff(buff.buff_id, unit.id, 100028, buff);
					healnum = healnum +1;				
				end
			end
