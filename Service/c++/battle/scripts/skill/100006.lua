...
{
	--九祸
	--对目标单体造成 340%伤害     破天邪印	附加咒术效果，持续2回合
	--特效屏蔽
	-- 名称
		name = '九祸',
	-- 优先级
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100006.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100006.buffs[1];
		...
		local attack_parameter = g_skill_100006.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100006.grow;
		local buff_trigger_rate = g_skill_100006.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				target:add_buff(buff.buff_id, unit.id, 100006, buff);
			end    
		end
