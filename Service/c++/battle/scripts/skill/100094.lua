...
{
	-- 天來眼
	--对前排全体造成 174% 伤害  	阴流沙	技能释放时有30%概率使目标眩晕，持续1回合
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100094.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100094.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100094.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100094.grow;
		local buff_trigger_rate = g_skill_100094.buff_trigger_rate;
				...
				-- 特殊效果
					if( unit.up < 4 ) then
						...
						if buff_trigger == true then
							--恶鬼罗刹效果
							if v.sid > 10001000 and v.sid < 10010000 then

							else
								v:add_buff(buff.buff_id, unit.id, 100094, buff);
							end
						end    
					end  
			end
