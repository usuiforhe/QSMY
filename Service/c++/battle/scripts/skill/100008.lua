...
{
	--箫中剑
	--对随机3个目标造成 175% 伤害       天意无私	技能30%概率对随机3个目标造成225%伤害
	-- 名称
		name = '箫中剑',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100008.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100010.buffs[1];
		...
		local attack_parameter = g_skill_100008.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100008.grow;
		local buff_trigger_rate = g_skill_100008.buff_trigger_rate;
	...
	--攻击范围
			...
			elseif (buff_trigger == true) then
				local damage= (unit.attack- target.defense) *( attack_parameter  +  grow * (skill_lv-1)+ 0.6)*(1-target.skill_resist/100)  + unit.event_hurt - target.event_injury      ;
				target:taking_damage(unit, damage, false);
			else
				local damage= (unit.attack- target.defense) *( attack_parameter  +  grow * (skill_lv-1))*(1-target.skill_resist/100) + unit.event_hurt - target.event_injury      ;
				target:taking_damage(unit, damage, false);
