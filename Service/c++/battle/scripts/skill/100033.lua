...
{
	-- 伊达我流
	--对后排全体造成 126% 伤害	 遁隐术	回避率+10% ，持续2回合
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100033.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100033.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100033.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100033.grow;
		local buff_trigger_rate = g_skill_100033.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(buff.buff_id, unit.id, 100033, buff);
			end		
		end
