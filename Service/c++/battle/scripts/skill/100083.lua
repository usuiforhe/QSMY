...
{
	--暴風殘道
	--对随机3个目标造成 169% 伤害      蚀骨之壤 	技能释放时有60%概率对目标附加中毒效果，持续2回合
	--特效屏蔽
	-- 名称
		name = '暴風殘道',
	-- 优先级
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100083.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100083.buffs[1];
		...
		local attack_parameter = g_skill_100083.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100083.grow;
		local buff_trigger_rate = g_skill_100083.buff_trigger_rate;
		...
		--攻击范围
			...
			if( unit.up < 4 ) then
				...
				if buff_trigger == true then
					target:add_buff(buff.buff_id, unit.id, 100083, buff);
				end
			end					
    			i = i + 1;
    		end
