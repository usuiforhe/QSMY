...
{
	--熏池
	--对友方所有目标73%治疗量	生命之源	 技能发动时会对所有友方单位释放恢复术，持续2回合
	-- 名称
		name = '熏池',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100089.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100089.buffs[1];
		local target = unit;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100089.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100089.grow;
		local buff_trigger_rate = g_skill_100089.buff_trigger_rate;
...
--		local damage= (unit.attack *( attack_parameter  +  grow * (skill_lv-1)))     ;
		...
		elseif (buff_trigger == true) then
			while ( healnum <= total ) do	
				...
				target:add_buff(buff.buff_id, unit.id, 100089, buff);
				healnum = healnum +1;				
			end
		end
