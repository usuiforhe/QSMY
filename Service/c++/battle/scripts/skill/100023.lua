...
{
	--伏嬰師
	--对所有目标造成101% 伤害，10%概率附加灼烧效果，持续2回合	岚火印	60%概率附加灼烧效果，持续2回合
	-- 特效屏蔽
	-- 名称
		name = '伏嬰師',
	-- 优先级
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100023.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100023.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100023.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100023.grow;
		local buff_trigger_rate = g_skill_100023.buff_trigger_rate;
		local buff_trigger = false ;
			...
			if( unit.up < 4 ) then
				...
				if buff_trigger == true then
					target:add_buff(buff.buff_id, unit.id, 100023, buff);
				end
			end				
