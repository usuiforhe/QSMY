...
{
	--悲曲绝世尘
	--对随机3个目标造成 157% 伤害     绝尘悲鸣	30%概率冰冻效果，持续2回合
	-- 名称
		name = '悲曲绝世尘',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100035.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100035.buffs[1];
		...
		local attack_parameter = g_skill_100035.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100035.grow;
		local buff_trigger_rate = g_skill_100035.buff_trigger_rate;
	...
	--攻击范围
			...
			if( unit.up < 4 ) then
				...
				--恶鬼罗刹效果
					...
					target:add_buff(buff.buff_id, unit.id, 100035, buff);
					end
				end   
			end
