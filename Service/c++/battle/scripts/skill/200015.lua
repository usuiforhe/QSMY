...
{
	--張墨&&阿西
	--对全体造成140%伤害      万鹊动力	使对方攻击下降10%，持续2回合
	-- 名称
		name = '万鹊动力',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_200015.trigger_rate;
		...
		local s1 = unit.force:get_unit_by_sid_all(110);
		local s2 = unit.force:get_unit_by_sid_all(111);
	...
	effect = function(frames, level, unit , attacker)
		local buff = g_skill_200015.buffs[1];
		local target = unit.target;	
		if target == nil then
		    return;
		end

		local buff_trigger_rate = g_skill_200015.buff_trigger_rate;
		local buff_trigger = false ;

		local members = {unit.force:get_unit_by_sid_all(110),unit.force:get_unit_by_sid_all(111)};
		...
		local attack_parameter = g_skill_200015.attack_parameter;
		local grow =  g_skill_200015.grow;
		...
		local units = unit.force.opponent:get_unit_list_table(true);
			...
			target:add_buff(buff.buff_id, unit.id, 200015, buff);
    			i = i + 1;
    		end
