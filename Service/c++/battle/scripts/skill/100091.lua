...
{
	-- 朱聞蒼日
	--对前排全体造成 174% 伤害  	逆天风	 技能释放时60%概率对所有敌方目标 -10% 防御，持续2回合
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100091.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100091.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100091.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100091.grow;
		local buff_trigger_rate = g_skill_100091.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				local unitsforce = unit.force.opponent:get_unit_list_table(true);
				local kk = 1;
				local total2 = table.getn(unitsforce);
				while kk <= total2 do
					local targetforce = unitsforce[kk];
					targetforce:add_buff(buff.buff_id, unit.id, 100091, buff);	
					kk = kk + 1;
				end --while
			end --if
		end
