...
{
	-- 恨長風
	--对前排全体造成 180% 伤害  	怒马凌关	技能释放时有60%概率对目标附加詛咒效果，持续2回合
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100085.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100085.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100085.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100085.grow;
		local buff_trigger_rate = g_skill_100085.buff_trigger_rate;
			...
			for id,v in pairs(targets2) do
				...
				if( unit.up < 4 ) then
					...
					if buff_trigger == true then
						v:add_buff(buff.buff_id, unit.id, 100085, buff);
					end   
				end
			end
		else
