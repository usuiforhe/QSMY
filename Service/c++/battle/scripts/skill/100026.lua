...
{
	-- 苏苓
	--对友方前排目标 144% 治疗量	满天花潮	友方所有增加5%攻击和5%防御，持续2回合
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100026.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100026.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100026.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100026.grow;
		local buff_trigger_rate = g_skill_100026.buff_trigger_rate;
		local buff_trigger = false ;
--		local damage= (unit.attack *( attack_parameter  +  grow * (skill_lv-1)))   ;			

    		local targets1 = {};
		local targets2 = {};
		local opponent = unit.force;
		...
		if ( flag == false ) then
			for id,v in pairs(targets2) do
				local damage= (unit.attack *( attack_parameter  +  grow * (skill_lv-1) +  v.base_heal_rate/100 ))   ;
				v:heal(unit,damage,false );
			end
		else
			for id,v in pairs(targets1) do
				local damage= (unit.attack *( attack_parameter  +  grow * (skill_lv-1) +  v.base_heal_rate/100 ))   ;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				...
				while kk <= total2 do
					...
					targetforce:add_buff(g_skill_100026.buffs[1].buff_id, unit.id, 100026, g_skill_100026.buffs[1]);	
					targetforce:add_buff(g_skill_100026.buffs[2].buff_id, unit.id, 100026, g_skill_100026.buffs[2]);	
					kk = kk + 1;
				end --while
			end --if
		end
