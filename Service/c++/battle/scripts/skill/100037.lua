...
{
	-- 草一色		
	--对三行目标造成 162% 伤害		福禄寿	暴击+20%，持续2回合
	-- 名称
		name = '草一色',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100037.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100037.buffs[1];		
		local attack_parameter = g_skill_100037.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100037.grow;
			
		local buff_trigger_rate = g_skill_100037.buff_trigger_rate;
	  ...
	  elseif unit.position <= 6 then 
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(buff.buff_id, unit.id, 100037, buff);
			end
		end
