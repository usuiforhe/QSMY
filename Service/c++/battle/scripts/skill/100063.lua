...
{
	-- 卓東來
	--对前排全体造成 168% 伤害  	惊风破	技能释放时使自己格挡率 +30%，持续2回合
	-- 名称
		name = '卓東來',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100063.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100063.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100063.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100063.grow;
		local buff_trigger_rate = g_skill_100063.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(buff.buff_id, unit.id, 100063, buff);
			end
		end
