...
{
	--四非凡人
	--对随机3个目标造成 157% 伤害     天坠飞星	技能释放时有20%概率使目标眩晕，持续1回合
	...
	--攻击系数
		attack_parameter = 1.57,
	--成长
		grow = 0.02,
	--buff触发几率
		buff_trigger_rate = 20,
	-- 验证该技能是否能够触发
	-- frames: 施法者回合数
	-- level: 技能等级
	-- unit: 拥有该技能的单位
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100041.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100041.buffs[1];
		...
		local attack_parameter = g_skill_100041.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100041.grow;
		local buff_trigger_rate = g_skill_100041.buff_trigger_rate;
	...
	--攻击范围
			...
			if( unit.up < 4 ) then
				...
				--恶鬼罗刹效果
					...
					target:add_buff(buff.buff_id, unit.id, 100041, buff);
					end
				end    
			end
    			i = i + 1;
