...
{
	-- 京極鬼彥		
	--对三行目标造成 174% 伤害		怒火燎原	如当前生命比目标单位高，那么伤害+25%		OK
	-- 名称
		name = '京極鬼彥',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100010.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100010.buffs[1];		
		local attack_parameter = g_skill_100010.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100010.grow;
				
		local buff_trigger_rate = g_skill_100010.buff_trigger_rate;
...
--[[		buff_trigger = script_check_prob(buff_trigger_rate);
	  ...
	  elseif unit.position <= 6 then 
		...
		for id,target in pairs(targets) do
			...
			elseif( unit.hp < target.hp	) then	
				local damage= (unit.attack- target.defense) *( attack_parameter  +  grow * (skill_lv-1))*(1-target.skill_resist/100)  + unit.event_hurt - target.event_injury   ;
				target:taking_damage(unit,damage ,false);
			else
				local damage= (unit.attack- target.defense) *( attack_parameter  +  grow * (skill_lv-1) + 0.25)*(1-target.skill_resist/100)  + unit.event_hurt - target.event_injury   ;
				target:taking_damage(unit,damage ,false);
			end	
		end
