...
{
	--疏樓龍宿
	--对目标单体造成 352%伤害    冰驰名封	技能有60%概率对目标附加冰冻效果，永久持续
	-- 名称
		name = '疏樓龍宿',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100082.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		...
		local buff = g_skill_100082.buffs[1];
		...
		local attack_parameter = g_skill_100082.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100082.grow;
		local buff_trigger_rate = g_skill_100082.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			--恶鬼罗刹效果
				...
				target:add_buff(buff.buff_id, unit.id, 100082, buff);
				end
			end
		end
