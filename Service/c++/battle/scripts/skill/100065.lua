...
{
	--赤雲染
	--对目标单体造成 304%伤害    玄流无音	技能释放时使目标防御 -20%，持续2回合
	-- 名称
		name = '赤雲染',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100065.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		...
		local buff = g_skill_100065.buffs[1];
		...
		local attack_parameter = g_skill_100065.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100065.grow;
		local buff_trigger_rate = g_skill_100065.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				target:add_buff(buff.buff_id, unit.id, 100065, buff);
			end
		end
