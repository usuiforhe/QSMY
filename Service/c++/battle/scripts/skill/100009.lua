g_skill_100009 =
{
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100009.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100009.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100009.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100009.grow;
		local buff_trigger_rate = g_skill_100009.buff_trigger_rate;
