...
{
	-- 劍子仙跡
	--对前排全体造成 186% 伤害        天下无双	技能有20%概率技能变为全体攻击
	-- 名称
		name = '劍子仙跡',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100081.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100081.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100081.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100081.grow;
		local buff_trigger_rate = g_skill_100081.buff_trigger_rate;
