g_skill_100057 =
{
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100057.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100057.buffs[1];
		...
		local attack_parameter = g_skill_100057.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100057.grow;
		local buff_trigger_rate = g_skill_100057.buff_trigger_rate;
	...
	--攻击范围
			...
			if( unit.up < 4 ) then
				...
				if buff_trigger == true then
					target:add_buff(buff.buff_id, unit.id, 100057, buff);
				end
			end					
    			i = i + 1;
    		end
