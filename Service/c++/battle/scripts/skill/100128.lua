...
{
	--兒道裘
	--对目标单体造成 340%伤害    罗布弯刀流 	技能释放时使目标眩晕，持续1回合
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100128.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		...
		local buff = g_skill_100128.buffs[1];
		...
		local attack_parameter = g_skill_100128.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100128.grow;
		local buff_trigger_rate = g_skill_100128.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if target.sid > 10001000 and target.sid < 10010000 then
				...
				target:add_buff(buff.buff_id, unit.id, 100128, buff);
			end
			end    
		end
