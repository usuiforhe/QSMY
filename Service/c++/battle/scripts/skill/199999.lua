g_skill_100050 =
{
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100050.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100010.buffs[1];
		...
		local attack_parameter = g_skill_100050.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100050.grow;
		local buff_trigger_rate = g_skill_100050.buff_trigger_rate;
		local buff_trigger = false ;
	
		local damage= (unit.attack *( attack_parameter  +  grow * (skill_lv-1))) - target.defense     ;
		target:taking_damage(unit, damage, true); 
