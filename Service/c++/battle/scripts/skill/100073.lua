...
{
	-- 螣邪郎	
	--对三行目标造成 174% 伤害    邪乂斩	技能释放后自身回避率+25% ，持续2回合
	-- 名称
		name = '赦生童子',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100073.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100073.buffs[1];		
		local attack_parameter = g_skill_100073.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100073.grow;
				
		local buff_trigger_rate = g_skill_100073.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(buff.buff_id, unit.id, 100073, buff);
			end		
		end
