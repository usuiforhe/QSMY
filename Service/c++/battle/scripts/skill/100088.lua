...
{
	-- 臥佛		
	--对三行目标造成 174% 伤害	云相法谛	技能50%触发无敌效果，持续1回合
	-- 名称
		name = '臥佛',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100088.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100088.buffs[1];		
		local attack_parameter = g_skill_100088.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100088.grow;
			
		local buff_trigger_rate = g_skill_100088.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(buff.buff_id, unit.id, 100088, buff);
			end
		end
