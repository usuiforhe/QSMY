...
{
	--風之痕
	--对所有目标造成104% 伤害    天外有天	技能释放时使友方所有闪避率 +10%，持续2回合
	-- 名称
		name = '風之痕',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100096.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100096.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100096.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100096.grow;
		local buff_trigger_rate = g_skill_100096.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				...
				while kk <= total2 do
					...
					targetforce:add_buff(buff.buff_id, unit.id, 100096, buff);	
					kk = kk + 1;
				end --while
			end --if
		end
