...
{
	--翠山行&&白雪飘
	--对随机三人造成伤害，如果目标血量高于自身，则额外附加30%伤害    双式合流破魔招	
	-- 名称
		name = '双式合流破魔招',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_200008.trigger_rate;
		...
		local s1 = unit.force:get_unit_by_sid_all(64);
		local s2 = unit.force:get_unit_by_sid_all(66);
	...
	effect = function(frames, level, unit , attacker)	
		...
		local members = {unit.force:get_unit_by_sid_all(64),unit.force:get_unit_by_sid_all(66)};
		...
		local attack_parameter = g_skill_200008.attack_parameter;
		local grow =  g_skill_200008.grow;
		...
		local units = unit.force.opponent:get_unit_list_table(true);
			...
			if target.hp<unit.hp then
				target:taking_damage(unit, damage , false);
			else
				target:taking_damage(unit, damage*1.3 , false);
    			end
			i = i + 1;
    		end
