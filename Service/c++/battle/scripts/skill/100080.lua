...
{
	-- 佛劍分說		
	--对随机3个目标造成 181% 伤害		杀生道	      技能释放时恢复自身最大生命值的50%
	-- 名称
		name = '佛劍分說',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100080.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		...
		local attack_parameter = g_skill_100080.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100080.grow;
		local buff_trigger_rate = g_skill_100080.buff_trigger_rate;
