...
{
	--月漩涡
	--对随机3个目标造成 163% 伤害      月下杀手	最终伤害+400
	-- 名称
		name = '月漩涡',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100034.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100010.buffs[1];
		...
		local attack_parameter = g_skill_100034.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100034.grow;
		local buff_trigger_rate = g_skill_100034.buff_trigger_rate;
	...
	--攻击范围
			...
			if( unit.up < 4 ) then
				...
				local damage2 = target.hp * 0.1;
				local damage= (unit.attack- target.defense)*( attack_parameter  +  grow * (skill_lv-1))*(1-target.skill_resist/100)   + damage2 + unit.event_hurt - target.event_injury    ;
				target:taking_damage(unit, damage, false);	
			end
