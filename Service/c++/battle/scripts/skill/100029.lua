...
{
	--千流影
	--对所有目标造成95% 伤害    云光回千影	回避率+20% ，持续2回合
	--特效屏蔽
	-- 名称
		name = '天问禁式',
	-- 优先级
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100029.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100029.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100029.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100029.grow;
		local buff_trigger_rate = g_skill_100029.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(buff.buff_id, unit.id, 100029, buff);
			end		
		end
