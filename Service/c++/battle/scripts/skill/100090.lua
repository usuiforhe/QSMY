...
{
	--魔化赭杉軍
	--对随机3个目标造成 163% 伤害       魔化玄气	技能释放时使自己回避率 +20%，持续2回合
	-- 名称
		name = '魔化赭杉軍',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100090.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100090.buffs[1];
		...
		local attack_parameter = g_skill_100090.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100090.grow;
		local buff_trigger_rate = g_skill_100090.buff_trigger_rate;
	...
	--攻击范围
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(buff.buff_id, unit.id, 100090, buff);
			end    
		end
