g_skill_100021 =
{
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100021.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker) 	
		local buff = g_skill_100021.buffs[1];		
		local attack_parameter = g_skill_100021.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100021.grow;
				
		local buff_trigger_rate = g_skill_100021.buff_trigger_rate;
	  ...
	  elseif unit.position <= 6 then 
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				unit:add_buff(buff.buff_id, unit.id, 100021, buff);
			end
		end
