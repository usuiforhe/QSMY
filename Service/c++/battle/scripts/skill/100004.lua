...
{
	--柳生剑影
	--对所有目标造成 104% 伤害    万神劫·封	对敌方随机2单位封技效果，持续1回合
	-- 名称
		name = '柳生剑影',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100004.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100004.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100004.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100004.grow;
		local buff_trigger_rate = g_skill_100004.buff_trigger_rate;
		...
		elseif (buff_trigger == true) then
			...
			if total > 1  then
				while ( num <3 ) do	
					local special = units[num];

					special:add_buff(buff.buff_id, unit.id, 100004, buff);
					num = num +1;		
				end
			else 
				units[1]:add_buff(buff.buff_id, unit.id, 100004, buff);
