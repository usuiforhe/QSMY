...
{
	--拳皇
	--对随机3个目标造成 175% 伤害      倾楼城	技能释放后自身 +50%格挡率，持续2回合
	--特效屏蔽
	-- 名称
		name = '拳皇',
	-- 优先级
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100059.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100059.buffs[1];
		...
		local attack_parameter = g_skill_100059.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100059.grow;
		local buff_trigger_rate = g_skill_100059.buff_trigger_rate;
	...
	--攻击范围
			...
			if( unit.up < 4 ) then
				...
				if buff_trigger == true then
					unit:add_buff(buff.buff_id, unit.id, 100059, buff);
				end
			end					
    			i = i + 1;
    		end
