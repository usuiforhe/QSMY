...
{
	--真田龙政
	--对随机3个目标造成 169% 伤害      夜狐百赦	友方所有增加5%回避，持续2回合
	-- 名称
		name = '真田龙政',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100027.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100027.buffs[1];
		...
		local attack_parameter = g_skill_100027.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100027.grow;
		local buff_trigger_rate = g_skill_100027.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				...
				while kk <= total2 do
					...
					targetforce:add_buff(buff.buff_id, unit.id, 100027, buff);	
					kk = kk + 1;
				end --while
			end --if
		end
