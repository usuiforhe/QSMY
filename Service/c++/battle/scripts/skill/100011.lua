g_skill_100011 =
{
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100011.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100011.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100011.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100011.grow;
		local buff_trigger_rate = g_skill_100011.buff_trigger_rate;
			...
			for id,v in pairs(targets2) do
				...
				if( unit.up < 4 ) then
					...
					if buff_trigger == true then
						v:add_buff(buff.buff_id, unit.id, 100011, buff);
					end
				end
			end	
		else
