...
{
	--樱千代
	--对全体造成 58% 伤害		秋荻映江色	清除友方所有DEBUFF
	-- 名称
		name = '樱千代',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100056.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100056.buffs[1];
		local target = unit;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100056.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100056.grow;
		local buff_trigger_rate = g_skill_100056.buff_trigger_rate;
