...
{
	-- 补剑缺		
	--对三行目标造成 174% 伤害		哎哟喂	将伤害40%转化为生命值
	-- 名称
		name = '补剑缺',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100022.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100022.buffs[1];		
		local attack_parameter = g_skill_100022.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100022.grow;
				
		local buff_trigger_rate = g_skill_100022.buff_trigger_rate;
...
--[[		buff_trigger = script_check_prob(buff_trigger_rate);
	  ...
	  elseif unit.position <= 6 then 
		...
		if( unit.up < 4) then
			...
			unit:heal(unit,count/i*0.4 , true);			
		end
