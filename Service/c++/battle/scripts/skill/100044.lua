...
{
	--御行者
	--对所有目标造成95% 伤害    灵焰蚀原        30%概率附加灼烧效果，持续2回合
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100044.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100044.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100044.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100044.grow;
		local buff_trigger_rate = g_skill_100044.buff_trigger_rate;
		local buff_trigger = false ;
			...
			if( unit.up < 4 ) then
				...
				if buff_trigger == true then
					target:add_buff(buff.buff_id, unit.id, 100044, buff);
				end
			end	
