...
{
	--弃天帝
	--对所有目标造成 115% 伤害    神之灭  技能释放时有3%概率对目标一击必杀
	-- 名称  
		name = '弃天帝',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		...
		local trigger_rate = g_skill_100001.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	--	local buff = g_skill_100001.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100001.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100001.grow;
		local buff_trigger_rate = g_skill_100001.buff_trigger_rate;
