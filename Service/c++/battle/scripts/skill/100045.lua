...
{
	-- 荻少将
	--对友方前排目标 132% 治疗量	冥鬼屋	60%概率对所有友方防御+5%，持续2回合
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100045.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100045.buffs[1];
		local target = unit.target;
		if target == nil then
		    return;
		end
		local target_id = target.id;
		local attack_parameter = g_skill_100045.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100045.grow;
		local buff_trigger_rate = g_skill_100045.buff_trigger_rate;
		...
		if( unit.up < 4 ) then
			...
			if buff_trigger == true then
				...
				while kk <= total2 do
					...
					targetforce:add_buff(buff.buff_id, unit.id, 100045, buff);	
					kk = kk + 1;
				end --while
			end --if
		end
