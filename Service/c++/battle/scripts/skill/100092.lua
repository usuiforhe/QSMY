...
{
	--文不通
	--对目标单体造成 316%伤害     龟甲兽文	30%概率触发冰冻效果，持续2回合
	-- 名称
		name = '文不通',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100092.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100092.buffs[1];
		...
		local attack_parameter = g_skill_100092.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100092.grow;
		local buff_trigger_rate = g_skill_100092.buff_trigger_rate;
	...
	--技能特殊效果	随机添加DEBUFF
		...
		if( unit.up < 4 ) then
			...
			--恶鬼罗刹效果
				...
				target:add_buff(buff.buff_id, unit.id, 100092, buff);
				end
			end    
		end
