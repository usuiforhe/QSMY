...
{
	--玄貘
	--对目标单体造成 352%伤害      萬滅伏神擊	技能释放时对目标附加冰冻和灼烧效果
	-- 名称
		name = '玄貘',
	-- 优先级
		priority = 1 ,
	...
	is_trigger = function(frames, level, unit, event_id, event_args)
		local trigger_rate = g_skill_100098.trigger_rate;
		local result =
		{
		    trigger = false,
		    coactors = {},
	...
	effect = function(frames, level, unit , attacker)	
		local buff = g_skill_100098.buffs[1];
		...
		local attack_parameter = g_skill_100098.attack_parameter;
		local skill_lv = level;
		local grow =  g_skill_100098.grow;
		local buff_trigger_rate = g_skill_100098.buff_trigger_rate;
	...
	--技能特殊效果	随机添加DEBUFF
		...
		if( unit.up < 0 ) then
			...
			--恶鬼罗刹效果
				...
				target:add_buff(g_skill_100098.buffs[1].buff_id, unit.id, 100098, g_skill_100098.buffs[1]);
				target:add_buff(g_skill_100098.buffs[2].buff_id, unit.id, 100098, g_skill_100098.buffs[2]);
				end
			end   
		end
