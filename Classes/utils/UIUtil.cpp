#include "UIUtil.h"
#include "../LuaProxy/LuaEventHandler.h"
#include "../AppMacros.h"
#include "../AppConsts.h"
...
void UIUtil::setOpacityRecursive( CCNode* pNode, unsigned int opacity )
{
	...
	CCARRAY_FOREACH(children, pObject)
	{
		if (pObject)
		{
			...
			if (child)
			{
				CCArray* nodeChildren = child->getChildren();
				if (nodeChildren && nodeChildren->count() > 0)
				{
					setOpacityRecursive(child, opacity);
				}
			}
			
			CCRGBAProtocol* pRGBNode = dynamic_cast<CCRGBAProtocol*>(child);
			if (pRGBNode)
			{
				pRGBNode->setOpacity(opacity);
			}
		}
	}
}

void UIUtil::runPopUpAction( CCNode* pNode, float duration /*= 0.3f*/ )
{
	if (pNode == NULL)
	{
		return;
	}
	pNode->setAnchorPoint(D_ANCHOR_POINT_CENTER);
	pNode->setPosition(D_DESIGN_POINT_CENTER);
	pNode->setScale(0.0f);
	pNode->runAction(CCEaseBackOut::create(CCScaleTo::create(duration, 1.f)));
}
