...
GuildBossRankCellRescue::GuildBossRankCellRescue()
	: m_pBMFStar(NULL)
	, m_pBMFRank(NULL)
	, m_pTTFName(NULL)
	, m_pNodeHead(NULL)
...
bool GuildBossRankCellRescue::init(cocos2d::extension::CCData *pCCBData)
{
	...
	m_pHelpNode->setVisible(true);
	m_pContributeNode->setVisible(false);
	return true;
}

bool GuildBossRankCellRescue::onAssignCCBMemberVariable(cocos2d::CCObject* pTarget, const char* pMemberVariableName, cocos2d::CCNode* pNode)
{
	CCB_MEMBERVARIABLEASSIGNER_GLUE(this, "m_pBMFRescue", CCLabelBMFont *, m_pBMFStar);
...
void GuildBossRankCellRescue::setData(CCObject *pData, uint32_t iRank, bool bChangeVisible, bool bLastWeek)
{
	...
	if (!pUser)
	{
		CCAssert(NULL != pUser, "Data error of GuildBossRankCellRescue");
		return;
	}
	m_pBMFStar->setString(D_CSTRING_FROM_UINT(pUser->getRescueCount()));
	...
	m_pBMFRank->setString(D_CSTRING_FROM_UINT(pUser->getRescueRank()));
}